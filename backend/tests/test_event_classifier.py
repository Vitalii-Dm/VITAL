...
def _vision(horizontal=False, fall=False) -> VisionSignal:
    return VisionSignal(
        flag=horizontal or fall,
        score=0.9 if horizontal else (0.6 if fall else 0.0),
        reason="",
...
def test_horizontal_plus_heat_is_heat_exhaustion():
    r = classify_event(
        _wifi(), _vision(horizontal=True), _env(heat=True), Severity.HIGH
    )
...
def test_horizontal_plus_breathing_anomaly_is_cardiac():
    r = classify_event(
        _wifi(breathing_anom=True), _vision(horizontal=True), _env(), Severity.MEDIUM
    )
...
def test_horizontal_alone_is_fall():
    # Medium from e.g. heat + horizontal? use transient fall here instead.
    r = classify_event(
        _wifi(), _vision(horizontal=True, fall=True), _env(heat=True), Severity.HIGH
    )
